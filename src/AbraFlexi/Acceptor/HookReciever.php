...
class HookReciever extends \AbraFlexi\Changes
{
     ...
     *
     * @return bool All Savers successed
     */
     ...
     */
    public function processAbraFlexiChanges(array $changes)
    {
        $changepos = 0;
        $doneIDd = [];
    ...
    public function saveLastProcessedVersion($version): void
    {
        ...
        if (null === $this->sqlEngine->insertToSQL(['serverurl' => $source, 'changeid' => $version])) {
    ...
    {
        return $this->isLocked() ? (int) (file_get_contents($this->lockfile)) : 0;
    }
