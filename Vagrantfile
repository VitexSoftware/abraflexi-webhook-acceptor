...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    export APACHE_DOCUMENT_ROOT=/usr/share/abraflexi-webhook-acceptor/
    ...
    #apt -y install abraflexi-webhook-acceptor-sqlite

    apt -y install mariadb-server
    systemctl start mysql
    apt -y install abraflexi-webhook-acceptor-mysql

    phinx seed:run -c /usr/lib/abraflexi-webhook-acceptor/phinx-adapter.php
    a2enconf abraflexi-webhook-acceptor
    ...
    links -dump http://localhost/abraflexi-webhook-acceptor
